...
int ptXsec() {
  ...
  double qcdnorm = 20.; //Set this using value from MET fit
  ...
  const int npar = 10;              // N-B in each bin, should match nbins
  TMinuit minuit(npar);
  minuit.SetFCN(fcn);
  ...
  int ierflg = 0;
  string parName[npar] = {"bin1", "bin2", "bin3", "bin4", "bin5", "bin6", "bin7", "bin8", "bin9", "bin10"};
  double par[npar];
  
  // Initialize parameters
  for(int i=0; i<npar; i++){ par[i] = (Ntotal - ini_val[0] - ini_val[1] - ini_val[2]) / 10.; }
  ...
  cout << xsec[0] << " for 300 GeV - 390 GeV " << endl;
  cout << xsec[1] << " for 390 GeV - 480 GeV " << endl;
  cout << xsec[2] << " for 480 GeV - 570 GeV " << endl;
  cout << xsec[3] << " for 570 GeV - 660 GeV " << endl;
  cout << xsec[4] << " for 660 GeV - 750 GeV " << endl;
  cout << xsec[5] << " for 750 GeV - 840 GeV " << endl;
  cout << xsec[6] << " for 840 GeV - 930 GeV " << endl;
  cout << xsec[7] << " for 930 GeV - 1020 GeV " << endl;
  cout << xsec[8] << " for 1020 GeV - 1110 GeV " << endl;
  cout << xsec[8] << " for 1110 GeV - 1200 GeV " << endl;
...
void getData(){
  ...
  //TFile* datafile = TFile::Open("Kinematic/JoshNtuples/Data_kin.root", "READ");
  TFile* datafile = TFile::Open("Kinematic/JoshNtuples/TTjets_POWHEG_inc_kin.root", "READ"); //using ttjets for closure test
  h_data = (TH1D*) gDirectory->Get("topTagptHist");
  h_data->Rebin(3);
...
double* getBkg(double qcdnorm){
  ...
  TString sample_array[8] = {"St_tW", "St_tWB", "St_t", "St_tB", "St_s", "St_sB", "WJets", "QCD"};
  TString sample_com_array[3] = {"stop", "wjets", "qcd"};
  TH1D** his_array = new TH1D*[8];
  ...
  for(int i=0; i<8; i++){
    file_array[i] = TFile::Open("Kinematic/JoshNtuples/"+sample_array[i]+"_kin.root", "READ");
    his_array[i] = (TH1D*) gDirectory->Get("topTagptHist");
    his_array[i]->Rebin(3);
  }
  ...
  // qcd
  double ntemp = his_array[7]->Integral();
  his_array[7]->Scale(qcdnorm / ntemp);
  his_com_array[2]->Add(his_array[7]);
  // I need to add in something here to subtract out signal contribution from shape...
