/* 
QCDminfit_comb.cc performs a maximum likelihood fit in the MET distribution to determine the amount of QCD background.  The fit parameters 
are the number of top and QCD events in the sample. These are used to normalize the respective templates in order to fit the MET 
distribution in data. Templates are taken from the MET histograms produced by plotMET.py, normalized to 1. The QCD template is further 
modified by subtracting the ttbar and w+jets sidebands before the normalization. The fit may be performed for any of the njets bins, or 
njets inclusive.
...
The inputs to be changed when running the program are found in lines 60, 78, 79, and 80. Line 60 gives the rebinning factor (number of 
bins in the original histogram to be combined before the fit), lines 78 and 79 give the njets bin and cutflow stage used to select the 
histograms for the fit, and line 80 gives the MC used to produce the TTjets template.
...
int QCDminfit_comb() {
  ...
  his_Temp[0]->SetTitle("");
  his_Temp[0]->Draw();
  double tempheight = min(1., 8. / nbins_new);
  his_Temp[0]->SetMaximum(tempheight);
  ...
  //hs->GetYaxis()->SetTitle("Events / %f GeV", 10. * binfac);
  result->Draw("same");
  data->Draw("same");
