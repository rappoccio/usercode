...
void plotUnfold() {
	...
	for (int is=0; is<nSYST; is++) {
		cout << "getting UnfoldingPlots/Unfolding_101214/unfolded_allsyst_nobackgroundSub/unfold_" << name_syst[is] << ".root" <<endl;
		f_syst[is] = new TFile("UnfoldingPlots/Unfolding_101214/unfolded_allsyst_nobackgroundSub/unfold_"+name_syst[is]+".root");
...
for (int i=0; i<h_count->GetNbinsX(); i++) {
  ...
  */
	...
	TCanvas *c1 = new TCanvas("c1", "", 700, 625);
	c1->SetTopMargin(0.10);
	c1->SetBottomMargin(0.15);
	c1->SetLeftMargin(0.13);
	c1->SetRightMargin(0.075);
	
	h_dummy_r->GetXaxis()->SetTitle("top Pt [GeV]");
	h_dummy_r->GetYaxis()->SetTitle("Relative Uncertainties");
	h_dummy_r->SetAxisRange(400,1400,"X");
	h_dummy_r->SetAxisRange(0,50,"Y");

	h_dummy_r->GetYaxis()->SetTitleSize(0.06);    
	h_dummy_r->GetYaxis()->SetTitleOffset(0.9);
	h_dummy_r->GetYaxis()->SetLabelSize(0.034);

	h_dummy_r->GetXaxis()->SetTitleSize(0.06);
	h_dummy_r->GetXaxis()->SetTitleOffset(0.8);
	h_dummy_r->GetXaxis()->SetLabelSize(0.034);
	c1->cd();
	...
	leg->Draw(); 
	c1->Draw();
	c1->SaveAs("unfold_relative_uncertainties.png");
}
