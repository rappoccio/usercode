...
do
	...
	do
		echo $LABEL $DIR 
		python TTHadronicAnalyzerCombined.py --dirs=/uscms_data/d2/jdolen/BoostedTop424Ntuple/CMSSW_4_2_4/src/Analysis/TTBSMPatTuples/test/${DIR}/res/ --outfile=TTHadronicAnalyzerCombined_${DIR} --analyzer=Type12Analyzer --mistagFile=MISTAG --useMC --collectionLabelSuffix=$LABEL
	done
done	
