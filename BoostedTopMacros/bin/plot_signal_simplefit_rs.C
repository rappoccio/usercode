...
void plot_signal()
{
  ...
  for ( int i = 0; i < nfiles; ++i ) {
    ...
    TH1 * hist_nontop_dijetmass = (TH1*)f->Get("hist_nontop_dijetmass");
    

    TH1D * hist_dijetmass = (TH1D*)hist_top_dijetmass->Clone();
    hist_dijetmass->SetName("hist_dijetmass");

    hist_dijetmass->Add( hist_nontop_dijetmass );

    hist_dijetmass->Sumw2();

    hist_dijetmass->SetFillColor(colors[i]);
    hist_dijetmass->Scale( Lum * xs[i] / (double)nentries[i] );
 
    hs->Add( hist_dijetmass );
//     leg->AddEntry( hist_dijetmass, titles[i], "f");


    double mean = hist_dijetmass->GetMean();
    double rms  = hist_dijetmass->GetRMS();
    funcs[i] = new TF1(fitnames[i],"gaus", bins[i][0], bins[i][1] );


    hist_dijetmass->Fit(funcs[i], "R");
//     funcs[i] = hist_dijetmass->GetFunction(fitnames[i]);
    ...
    TString outname1("fit_rs_");
    outname1 += fitnames[i];
    outname1 += ".gif";
    c0->Print(outname1.Data(), "gif");
    TString outname2("fit_rs_");
    outname2 += fitnames[i];
    outname2 += ".eps";
    c0->Print(outname2.Data(), "eps");
  }

  double resolution_x[nfiles] = { 750, 1000, 1250, 1500, 2000, 2500, 3000};
  double resolution_dx[nfiles]= {   0,    0,    0,    0,    0,    0,    0};
  ...
  leg->Draw();
  c1->SetLogy();
  c1->Print("resolution_fits_rs.gif", "gif");
  c1->Print("resolution_fits_rs.eps", "eps");
  ...
  foraxes2->Draw();
  foraxes2->SetMinimum(0.05);
  foraxes2->SetMaximum(0.2);
  foraxes2->GetYaxis()->SetTitleOffset(1.0);
  resolution->Draw("LP");
  gStyle->SetOptStat(000000);
  TF1 * resfit = new TF1("resfit", "[0] + [1]/(x*x*x)", 0, 5000);
  resolution->Fit("resfit", "R");

  c2->Print("resolution_function_rs.gif", "gif");
  c2->Print("resolution_function_rs.eps", "eps");
