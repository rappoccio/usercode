...
files.push_back("/uscms/home/rappocc/nobackup/TopTaggingV2/qcd_600_v6_fixed/antikt_pat_slim_223_9.root");
  ...
  TString fname( "subjet_studies_jec_" );
  fname += sample.c_str();
  fname += ".root";
  ...
  TH1D * hist_subjetPt1 = new TH1D("hist_subjetPt1", "Subjet Pt 1", 100, 0, 2000);
  TH1D * hist_subjetY1 = new TH1D("hist_subjetY1", "Subjet Y 1", 100, -5.0, 5.0);
  ...
  TH1D * hist_subjetPt2 = new TH1D("hist_subjetPt2", "Subjet Pt 2", 100, 0, 2000);
  TH1D * hist_subjetY2 = new TH1D("hist_subjetY2", "Subjet Y 2", 100, -5.0, 5.0);
  ...
  TH1D * hist_subjetPt3 = new TH1D("hist_subjetPt3", "Subjet Pt 3", 100, 0, 2000);
  TH1D * hist_subjetY3 = new TH1D("hist_subjetY3", "Subjet Y 3", 100, -5.0, 5.0);
  ...
  TH1D * hist_subjetPt4 = new TH1D("hist_subjetPt4", "Subjet Pt 4", 100, 0, 2000);
  TH1D * hist_subjetY4 = new TH1D("hist_subjetY4", "Subjet Y 4", 100, -5.0, 5.0);
  ...
  TH1D * min_mass_3subjets = new TH1D("min_mass_3subjets", "Min Mass, 3 subjets;Min Mass (GeV/c^{2})", 50, 0, 200);
  ...
  TH1D * min_mass_4subjets = new TH1D("min_mass_4subjets", "Min Mass, 4 subjets;Min Mass (GeV/c^{2})", 50, 0, 200);
  TH1D * min_3mass_4subjets = new TH1D("min_3mass_4subjets", "Min Mass of top 3 subjets, 4 subjets;Min Mass (GeV/c^{2})", 50, 0, 200);
  TH1D * min_3primemass_4subjets = new TH1D("min_3primemass_4subjets", "Min Mass, Merge lowest subjets, 4 subjets;Min Mass (GeV/c^{2})", 50, 0, 200);
  ...
  for( ev.toBegin();
    ...
    string Levels = "L2:L3";
    string Tags = "Summer08_L2Relative_KT4Calo:Summer08_L3Absolute_KT4Calo";
    ...
    for ( ; ijet != jetsend; ++ijet ) {
      ...
      hist_n_subjets->Fill( subjets.size() );

      PartonMatcher matcher( subjets, *h_genParticles );
      
      TLorentzVector jet_p4( ijet->px(), ijet->py(), ijet->pz(), ijet->energy() );

      int nsubjets = subjets.size();

      std::vector<TLorentzVector> subjets_p4;

      // do jec
//       cout << "Doing JEC" << endl;
      reco::Jet::Constituents::const_iterator isubjetPtrBegin = subjets.begin(),
	isubjetPtrEnd = subjets.end(), isubjetPtr = isubjetPtrBegin;
      for (; isubjetPtr != isubjetPtrEnd; ++isubjetPtr ) {
// 	cout << "   isubjet = " << isubjetPtr - isubjetPtrBegin << endl;

	reco::CaloJet const * isubjet_new = dynamic_cast<reco::CaloJet const*>( (*isubjetPtr).get() );
	if ( isubjet_new != 0 ) {
	
// 	  cout << "Succesful dynamic cast" << endl;
	  double pt = isubjet_new->pt();    
	  double eta = isubjet_new->eta();  
	  double emf = isubjet_new->emEnergyFraction();  
// 	  cout << "pt = " << pt << ", eta = " << eta << ", emf = " << emf << endl;
	  double L2L3scale = L2L3JetCorrector->getCorrection(pt,eta,emf);
	  
	  subjets_p4.push_back( TLorentzVector( isubjet_new->px() * L2L3scale,
						isubjet_new->py() * L2L3scale,
						isubjet_new->pz() * L2L3scale,
						isubjet_new->energy() * L2L3scale ) );
	}
      }
	...
	TLorentzVector subjet1_p4(subjets_p4[0] );
...
// 	hist_subjetFlavour1->Fill( matcher.getParton(0)->pdgId() );
	...
	if ( nsubjets > 1 ) {
	  TLorentzVector subjet2_p4(subjets_p4[1] );
...
// 	  hist_subjetFlavour2->Fill(matcher.getParton(1)->pdgId());
	  ...
	  if ( nsubjets > 2 ) {
	    TLorentzVector subjet3_p4(subjets_p4[2] );
...
// 	    hist_subjetFlavour3->Fill(matcher.getParton(2)->pdgId());
	    ...
	    if ( nsubjets > 3 ) {
	      TLorentzVector subjet4_p4(subjets_p4[3] );
...
// 	      hist_subjetFlavour4->Fill(matcher.getParton(3)->pdgId());
      ...
      // 	  cout << "Subjets : " << endl;
      vector<TLorentzVector>::const_iterator isubjetBegin = subjets_p4.begin(),
	isubjetEnd = subjets_p4.end(), isubjet = isubjetBegin;

      // Find closest matching subjet
      if ( subjets_p4.size() >= 3 ) {



	for ( isubjet = isubjetBegin; isubjet != isubjetEnd - 1; ++isubjet ) {
	  TLorentzVector const & isubjet_p4 = *isubjet;

	  vector<TLorentzVector>::const_iterator jsubjet = isubjet + 1;

	  for ( ; jsubjet != isubjetEnd; ++jsubjet ) {
	    TLorentzVector const & jsubjet_p4 = *jsubjet;
	...
	} else if ( subjets_p4.size() == 4 ) {
	  min_mass_4subjets->Fill( min_mass );
	}


	if ( subjets_p4.size() == 4 ) {
