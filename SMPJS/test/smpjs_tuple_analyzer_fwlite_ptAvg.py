...
for ifile in files :
    ...
    for event in events:
        ...
        for igroom in range(0,len(ak7Objs)):
            ...
            # Fake conditions:
            # 0. If there are fewer than two gen jets but at least two reco jets.
            # 1. If the leading two reco jets do not match to the leading two gen jets.
            # 1. If there are more than 2 reco jets (veto the trijet topology)
            if len(ak7Reco) >= 2 :
		...
		ptAsymmetry = (ak7Reco[0].Perp() - ak7Reco[1].Perp()) / (ak7Reco[0].Perp() + ak7Reco[1].Perp())
		if igroom == 0 :
			hists.ptAsymmetry.Fill( ptAsymmetry )
                ...
                fake2Condition = len(ak7GenMatched) >= 2 and \
                    ak7GenMatched[0] is not None and ak7GenMatched[1] is not None and \
                    (ak7GenIndices[0] not in [0,1] or ak7GenIndices[1] not in [0,1] )
                fake3Condition = len(ak7GenMatched) >= 2 and njetsAboveThreshold != 2
		fake4Condition = ptAsymmetry > 0.3
                
                if (fake0Condition or fake1Condition or fake2Condition or fake3Condition or fake4Condition ) :
                    isFake = True
                    response = responses[responsePtBin][igroom]
                    response.Fake( mjet )
            ...
            if len(ak7Reco) >= 2 and len(ak7GenMatched) >= 2 :
                ...
                #if (mjet > 50.0 and mjetGen < 50.0 and ptAvg < 125.0)
                if ( mjet < mjetGen - 30 and responsePtBin == 4 and ak7RecoMatched[0] is None):
