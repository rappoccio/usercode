    ...
    if test -f $INPATH/$txt; then
        ...
        if test $DATA -eq 0; then
            if test $counter -eq $PROCESS; then
                echo "python ntupleMaker.py --onDcache --txtfiles $INPATH/$txt --sample $OUTPATH/${name}_nominal"
                python ntupleMaker.py --onDcache --txtfiles $INPATH/$txt --sample $OUTPATH/${name}_nominal
            fi
            let "counter+=1"
        else
            let "counter+=1"
