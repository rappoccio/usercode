...
fTopFL = ROOT.TFile("TTSemilepAnalyzer_TTJetsLeptonic_TTWeight_ptWeighted_type1.root") 
fTopSL = ROOT.TFile("TTSemilepAnalyzer_TTJets_TTWeight_ptWeighted_type1.root")



fWMC = ROOT.TFile("TTSemilepAnalyzer_WJets_TTWeight_ptWeighted_type1.root")
fZJetsMC = ROOT.TFile("TTSemilepAnalyzer_ZJets_TTWeight_ptWeighted_type1.root")
fSingletop = [
ROOT.TFile("TTSemilepAnalyzer_ST_t_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tB_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tW_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tWB_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_s_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_sB_TTWeight_ptWeighted_type1.root"),
]
...
t12ddata.SetStats(0)
t12ddata.Draw('colz')
c2d.Print( 'leptoppt_vs_nsj_TTWeight.root', 'root')
c2d.Print( 'leptoppt_vs_nsj_TTWeight.pdf', 'pdf')
...
for hist2dn in hist2ds:
	...
	c2d1[j].Print( hist2dn+"_TTWeight.root", 'root')
	c2d1[j].Print( hist2dn+"_TTWeight.pdf", 'pdf')
	...
	c2d2[j].Print( hist2dn+"_TTWeight_bkg.pdf", 'pdf')
	c2d2[j].Print( hist2dn+"_TTWeight_bkg.root", 'root')
	...
	subt.Draw('colz')
	prelim.DrawLatex( 0.15, 0.91, "#scale[1.0]{CMS Preliminary #sqrt{s} = 8 TeV, 19.7 fb^{-1}}" )
	c2d3[j].Print( hist2dn+"_TTweight_sub.pdf", 'pdf')
	c2d3[j].Print( hist2dn+"_TTweight_sub.root", 'root')
	j+=1
if options.pt == 'high':
...
for t1hist in t1hists:
	...
	c.Print( 'semiLepMassNewSel_'+t1titles[i]+'_TTWeight.root', 'root')
	c.Print( 'semiLepMassNewSel_'+t1titles[i]+'_TTWeight.pdf', 'pdf')
	i+=1
