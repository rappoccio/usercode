...
fTopSL =  ROOT.TFile("TTSemilepAnalyzer_TTJets_"+MCstring+"_TTWeight_ptWeighted_type1.root")


fWMC = ROOT.TFile("TTSemilepAnalyzer_WJets_"+MCstring+"_TTWeight_ptWeighted_type1.root")
fZJetsMC = ROOT.TFile("TTSemilepAnalyzer_ZJets_"+MCstring+"_TTWeight_ptWeighted_type1.root")
fSingletop = [
ROOT.TFile("TTSemilepAnalyzer_ST_t_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tB_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tW_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_tWB_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_s_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
ROOT.TFile("TTSemilepAnalyzer_ST_sB_"+MCstring+"_TTWeight_ptWeighted_type1.root"),
]
...
t12ddata.SetStats(0)
t12ddata.Draw('colz')
c2d.Print( 'leptoppt_vs_nsj_'+MCstring+'_TTWeight.root', 'root')
c2d.Print( 'leptoppt_vs_nsj_'+MCstring+'_TTWeight.pdf', 'pdf')
...
for hist2dn in hist2ds:
	...
	c2d1[j].Print( hist2dn+"_"+MCstring+"_TTWeight.root", 'root')
	c2d1[j].Print( hist2dn+"_"+MCstring+"_TTWeight.pdf", 'pdf')
	...
	c2d2[j].Print( hist2dn+"_"+MCstring+"_TTWeight_bkg.pdf", 'pdf')
	c2d2[j].Print( hist2dn+"_"+MCstring+"_TTWeight_bkg.root", 'root')
	...
	subt.Draw('colz')
	prelim.DrawLatex( 0.15, 0.91, "#scale[1.0]{CMS Preliminary #sqrt{s} = 8 TeV, 19.7 fb^{-1}}" )
	c2d3[j].Print( hist2dn+"_"+MCstring+"_TTweight_sub.pdf", 'pdf')
	c2d3[j].Print( hist2dn+"_"+MCstring+"_TTweight_sub.root", 'root')
	j+=1
if options.pt == 'high':
...
i=0
output = ROOT.TFile( "ttbarNewSel_weight_INVALID.root", "recreate" )
output1 = ROOT.TFile( "ptlepNewSel_weight_INVALID.root", "recreate" )
output.cd()
...
for t1hist in t1hists:
	...
	if i==0:
		...
		leg.AddEntry( t1SLttbar, 't#bar{t}', 'f')
	...
	c.Print( 'semiLepMassNewSel_'+t1titles[i]+'_'+MCstring+'_TTWeight.root', 'root')
	c.Print( 'semiLepMassNewSel_'+t1titles[i]+'_'+MCstring+'_TTWeight.pdf', 'pdf')
	i+=1
